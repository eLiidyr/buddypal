local function helper(bp, events)
    ...
    local function updateFollowing(commands)
        if not ftarget then return end
        local target = bp.__target.get(ftarget)
    ...
    local function handleZonelines(id, original)
        if (not S{0x05E}:contains(id) or not ftarget) then return false end
        local parsed = bp.packets.parse('outgoing', original)
        
        if parsed then

            if ftarget and ftarget == bp.__player.id() then
                local update;
    ...
    local function handleFollowing(id, original)
        ...
        if prerender:ready() and ftarget then

            if ftarget == bp.__player.id() then
    ...
    local function handleStatus(n, o)
        ...
        if (S{1,2,3,4,38,39,40,41,42,43,44,47}):contains(n) and ftarget then
            cancelFollow()
