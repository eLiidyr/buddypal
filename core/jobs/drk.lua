local function load(bp, settings)
    ...
    function self:specials()
        ...
        if player and target and bp.core.get('one-hours') and bp.actions.canAct() and player.status == 1 then

            if bp.core.get('blood-weapon') and bp.core.ready("Blood Weapon", 51) then
                bp.queue.add("Blood Weapon", player)

            end
            
            if bp.core.get('soul-enslavement') and bp.core.ready("Soul Enslavement", 497) then
        ...
        if player and target and bp.core.get('drain') and bp.core.get('drain').enabled and bp.__player.hpp() < bp.core.get('drain').hpp then
            ...
            for drain in T{"Drain III","Drain II","Drain"}:it() do
                ...
                if bp.core.ready(drain) then
                    ...
                    if drain == "Drain III" and bp.core.get('dark-seal') and bp.core.ready("Dark Seal", 345) and bp.actions.canAct() then
    ...
    function self:abilities()
        ...
        if bp.core.get('job-abilities') and bp.actions.canAct() then
            ...
            if player and player.status == 1 then
                ...
                -- WEAPON BASH.
                if bp.core.get('weapon-bash') and bp.core.ready("Weapon Bash") and target then
        ...
        if bp.core.get('buffing') then
            ...
            if player and player.status == 1 then
                ...
                if bp.actions.canAct() and target then
                    ...
                    -- CONSUME MANA.
                    if bp.core.get('consume-mana') and bp.core.ready("Consume Mana", 599) and bp.__player.tp() >= bp.core.get('melee-weaponskill').tp then
                        bp.queue.add("Consume Mana", player)

                    end

                    -- LAST RESORT.
                    if not bp.core.get('tank-mode') and bp.core.get('last-resort') and bp.core.ready("Last Resort", 63) then
                        bp.queue.add("Last Resort", player)

                    -- SOULEATER.
                    elseif not bp.core.get('tank-mode') and bp.core.get('souleater') and bp.core.ready("Souleater", 64) then
                        bp.queue.add("Souleater", player)

                    -- DIABOLIC EYE.
                    elseif bp.core.get('diabolic-eye') and bp.core.ready("Diabolic Eye", 349) then
                        bp.queue.add("Diabolic Eye", player)

                    -- SCARLET DELIRIUM.
                    elseif bp.core.get('scarlet-delirium') and bp.core.ready("Scarlet Delirium", {479,480}) then
                ...
                if bp.actions.canCast() then
                    ...
                    if bp.core.get('absorb') and bp.core.get('absorb').enabled and target then
                        ...
                        if absorb and bp.core.ready(absorb) and (not bp.__buffs.hasAbsorb() or absorb == "Absorb-Attri" or absorb == "Absorb-TP") then
                            ...
                            -- DARK SEAL.
                            if bp.core.get('dark-seal') and bp.core.ready("Dark Seal", 345) and bp.actions.canAct() then
                                bp.queue.add("Dark Seal", player)
                            
                            end

                            -- NETHER VOID.
                            if bp.core.get('nether-void') and bp.core.ready("Nether Void", 439) and bp.actions.canAct() then
                    ...
                    -- DREAD SPIKES.
                    if bp.core.get('dread-spikes') and not bp.__buffs.hasSpikes() then
